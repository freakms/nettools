...
class NetToolsApp(ctk.CTk):
    ...
    def create_dashboard_content(self, parent):
        ...
        self.create_stat_card(
            ...
            lambda: self.show_page("scanner"),
            0, 0
        )
        
        # Card 2: Favorites
        fav_count = len(self.favorite_tools)
        self.create_stat_card(
            stats_frame,
            "⭐",
            "Favorites",
        ...
        recent_scans = len(self.scanner.results) if hasattr(self, 'scanner') else 0
        self.create_stat_card(
            stats_frame,
            "📊",
            "Recent Activity",
        ...
        self.create_quick_actions_section(left_col)
        
        # Recent Scans section
        self.create_recent_scans_section(left_col)
        ...
        self.create_favorite_tools_section(right_col)
        
        # Tips & Shortcuts section
        self.create_tips_section(right_col)
    
    def create_stat_card(self, parent, icon, title, subtitle, color, command, row, col):
    ...
    def create_quick_actions_section(self, parent):
        ...
        # Action buttons
        actions = [
            ("🔍 IPv4 Scanner", "Scan network for active hosts", lambda: self.show_page("scanner")),
            ("🔌 Port Scanner", "Check open ports on hosts", lambda: self.show_page("portscan")),
            ("🗺️ Traceroute", "Trace network path", lambda: self.show_page("traceroute")),
            ("🌐 DNS Lookup", "Resolve hostnames", lambda: self.show_page("dns")),
        ]
    ...
    def create_recent_scans_section(self, parent):
        ...
        if hasattr(self, 'scanner') and len(self.scanner.results) > 0:
            results_preview = self.scanner.results[:5]  # Show last 5
    ...
    def create_favorite_tools_section(self, parent):
        ...
        if self.favorite_tools:
            for tool_id in self.favorite_tools:
                ...
                btn_frame.bind("<Button-1>", lambda e, tid=tool_id: self.show_page(tid))
